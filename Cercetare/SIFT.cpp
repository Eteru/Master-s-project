...
cl::Image2D * SIFT::Run(cl::Image2D * image, uint32_t w, uint32_t h)
{
	...
	m_octaves.push_back(Octave(ref_img, crt_w, crt_h, NUMBER_OF_BLURS));
...
cl::Image2D *  SIFT::SetupReferenceImage(cl::Image2D * image, uint32_t w, uint32_t h)
{
	...
	try
	{
		...
		cl::Image2D resized_img = cl::Image2D(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, imf, 2 * w, 2 * h);
		cl::Kernel kernel_resize = *CLManager::GetInstance()->GetKernel(Constants::KERNEL_RESIZE);

		res = kernel_resize.setArg(0, *image);
		res = kernel_resize.setArg(1, resized_img);
